...
public class GaugeView extends View {
    ...
    public static final float NEEDLE_WIDTH = 0.018f;
...
//    private Bitmap mBackground;
    ...
    public void setTargetValue(final float value) {
        if (mShowScale || mShowRanges) {
            if (value < mScaleStartValue) {
                ...
                mTargetValue = otherDivisions(value);
            } else {

                if(value <= 10){ // 1 Mbps division
                    mTargetValue =  (float)(value * 12.5) / 10;     // correct
                    Log.d("Target",""+mTargetValue);
                } else{
                    if(value <= 20){ // 2 Mbps division

                        mTargetValue = (float) (12.5 + ((value - 10) * 0.3125));

                        //mTargetValue =  (float)((value * 15.625) / 10) / 2;     // correct
                        Log.d("Target",""+mTargetValue);
                    }else{
                        if(value <= 30){ // 3  Mbps division

                            mTargetValue = (float) (15.625 + ((value - 20) * 0.3125));

                            //mTargetValue =  (float)((value * 18.75) / 10) / 3;
                            Log.d("Target",""+mTargetValue);
                        }else{
                            if(value <= 40){// 4  Mbps division

                                mTargetValue = (float) (18.75 + ((value - 30) * 0.3125));

                                //mTargetValue =  (float)((value * 21.875) / 10) / 4;
                                Log.d("Target",""+mTargetValue);
                            }else{
                                if(value <= 50) {// 5  Mbps division

                                    mTargetValue = (float) (21.875 + ((value - 40) * 0.3125));

                                    //mTargetValue =  (float)((value * 25) / 10) / 5;
                                    Log.d("Target", "" + mTargetValue);
                                }else{
                                    if(value <= 100){ // 10 Mbps division
                                        mTargetValue = tenthDivision(value);
                                    }/*else{
                                        mTargetValue = otherDivisions(value);
                                    }*/
                                }
    ...
    private float tenthDivision(float value){

        if(value <= 60){ // 6 Mbps division

            mTargetValue = (float) (25 + ((value - 50) * 0.25));

            //mTargetValue =  (float)((value * 27.5) / 10) / 6;
            Log.d("Target", "" + mTargetValue);
        }else{
            if(value <= 70){ // 7 Mbps division

                mTargetValue = (float) (27.5 + ((value - 60) * 0.25));

                //mTargetValue =  (float)((value * 30) / 10) / 7;
                Log.d("Target", "" + mTargetValue);
            }else{
                if(value <= 80){ // 8 Mbps division

                    mTargetValue = (float) (30 + ((value - 70) * 0.25));

                    //mTargetValue =  (float)((value * 32.5) / 10) / 8;
                    Log.d("Target", "" + mTargetValue);
                }else{
                    if(value <= 90){ // 9 Mbps division

                        mTargetValue = (float) (32.5 + ((value - 80) * 0.25));

                        //mTargetValue =  (float)((value * 35) / 10) / 9;
                        Log.d("Target", "" + mTargetValue);
                    }else{
                        if(value <= 100){ // 10 Mbps division

                            mTargetValue = (float) (35 + ((value - 90) * 0.25));

                            //mTargetValue =  (float)((value * 37.5) / 10) / 10;
                            Log.d("Target", "" + mTargetValue);
                        }
                    }
                }
    ...
    private float otherDivisions(float value){
        if(value <= 200){ // 20 Mbps division

            mTargetValue = (float) (37.5 + ((value - 100) * 0.125));

            //mTargetValue =  (float)( (value * 50) /10) / 20;
            Log.d("Target",""+mTargetValue);
        } else {
            if(value <= 300){ // 30 Mbps division

                mTargetValue = (float) (50 + ((value - 200) * 0.125));

                //mTargetValue =  (float)( (value * 62.5) /10) / 30;
                Log.d("Target",""+mTargetValue);
            }else{
                if(value <= 500){ // 50 Mbps division

                    mTargetValue = (float) (62.5 + ((value - 300) * 0.0625));

                    //mTargetValue =  (float)( (value * 75) /10) / 50;
                    Log.d("Target",""+mTargetValue);
                }else{
                    if(value <= 750){ // 75 Mbps division

                        mTargetValue = (float) (75 + ((value - 500) * 0.05));

                        //mTargetValue =  (float)( (value * 87.5) /10) / 75;
                        Log.d("Target",""+mTargetValue);
                    }else{ // 100 Mbps division
                        if(value < 1000)

                            mTargetValue = (float) (87.5 + ((value - 750) * 0.05));

                        //mTargetValue =   value / 10;
                        Log.d("Target",""+mTargetValue);
                    }
                }
            }
    ...
    public Paint getDefaultNeedleLeftPaint() {
        ...
        paint.setColor(Color.rgb(26,35, 69));
        else
            paint.setColor(Color.rgb(159, 31, 57));
        return paint;
    }
    ...
    public Paint getDefaultNeedleScrewPaint() {
        final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setShader(new RadialGradient(0.5f, 0.5f, 0.04f, new int[]{Color.rgb(171, 171, 171), Color.WHITE}, new float[]{0.05f,
    ...
    private void computeCurrentValue() {
        ...
        if (-1 != mNeedleLastMoved) {
            ...
            if (Math.abs(mNeedleVelocity) < 90.0f) {
                mNeedleAcceleration = 3.0f * (mTargetValue - mCurrentValue);
            } else {
