...
public class EventsListArrayAdapter  extends ArrayAdapter<Event> {
	...
	public EventsListArrayAdapter(Activity context, Event[] newEvents) {
		super(context, R.layout.eventslist_item,newEvents );
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (view == null) {
			...
			view = vi.inflate(R.layout.eventslist_item, null,true);
		...
		if (MyApplication.nightMod){
			holder.imageView.setTag(events[position].getThumbnailUrlNight());
		}else{
			holder.imageView.setTag(events[position].getThumbnailUrlDayl());
		}
		try{
			//LoadImage mLoadImage=new LoadImage();
		//	mLoadImage.onPreExecute(holder);
		//	mLoadImage.execute(holder.imageView);
			try {
				if(MyApplication.nightMod)
...
//	protected class LoadImage extends AsyncTask<ImageView,Void,Bitmap>{
//		ViewHolder holder;
//
//
//		@Override
